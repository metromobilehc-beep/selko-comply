...
async function completeModule(score){
  ...
  // staff_id must stay as currentUser.id (== auth.uid()) — the RLS policy
  // on compliance_completions requires staff_id = auth.uid() exactly.
  // This used to look up compliance_staff and overwrite staffId with that
  // row's own id instead, which is a completely different UUID — worked
  // by accident for anyone whose compliance_staff lookup happened to fail
  // (no matching row), broke the moment a real compliance_staff record
  // existed to be found (exactly what happened for Gracen White once her
  // missing row was created). Only the display name comes from that
  // lookup now, never the id used for the actual database write.
...
async function downloadMyCertificate(moduleId){
  try{
    // Same fix as completeModule() — staff_id must stay currentUser.id
    // (== auth.uid()), matching how completions are actually stored,
    // never overwritten with compliance_staff's own id.
    let staffId = currentUser.id;
    let employeeName = currentProfile.full_name || currentProfile.email || '';
    const staffRes = await fetch(
      SUPABASE_URL + '/rest/v1/compliance_staff?company_id=eq.' + currentProfile.company_id + '&email=eq.' + encodeURIComponent(currentProfile.email) + '&select=id,full_name&limit=1',
